...
if (isset($_POST["login"])) {
  ...
  $lvl = addBranch("SELECT * FROM users WHERE username = '$user'")[0];
  $lvlUser = $lvl['level'];
  ...
  if (mysqli_num_rows($ambil) === 1) {
    ...
    if (password_verify($pass, $klm["password"])) {
      //set sesi
      $_SESSION["login"] = $klm["id"];

      if ($lvlUser == "admin") {
        header("location: admin/index.php");
        exit;
      } else if ($lvlUser == "operator") {
        header("location: indexOp.php");
        exit;
      }
    }
    $salah = true;
      ...
      <div class="card-body login-card-body">
        ...
        <form action="" method="post">
          ...
          <div class="input-group mb-3">
            <input name="username" id="username" type="text" required autofocus autocomplete="off" class="form-control" placeholder="Username">
          ...
          <div class="input-group mb-3">
            <input name="password" id="password" type="password" required autocomplete="off" class="form-control" placeholder="Password">
            <div class="input-group-append">
              <div class="input-group-text">
